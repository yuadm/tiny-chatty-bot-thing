...
-- First, let's update the user_roles table to include more role options
-- and add tables for permissions and branch restrictions

-- Add more role options to user_roles if not already present
DO $$
BEGIN
    -- Check if we need to add more roles
...
-- Create user permissions table for page/feature access control
...
-- Create user branch permissions table for branch-level access control
...
-- Update compliance_period_records to track who inputted the record
ALTER TABLE public.compliance_period_records 
ADD COLUMN IF NOT EXISTS inputted_by UUID REFERENCES auth.users(id),
ADD COLUMN IF NOT EXISTS inputted_at TIMESTAMP WITH TIME ZONE DEFAULT now();

-- Create function to get user display name (email for now, can be enhanced later)
CREATE OR REPLACE FUNCTION get_user_display_name(user_id UUID)
RETURNS TEXT
LANGUAGE sql
SECURITY DEFINER
AS $$
  SELECT COALESCE(email, 'Unknown User')
  FROM auth.users
  WHERE id = user_id;
$$;
